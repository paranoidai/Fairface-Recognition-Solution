...
class FaceImageIter(io.DataIter):
    ...
    def __init__(self, batch_size, data_shape,
                 ...
                 mx_model = None,
                 fairface_mode = False,
                 data_name='data', label_name='softmax_label', dis_type = 'e',  **kwargs):
        ...
        print('ids count: ', len(self.id2range))
        self.seq = self.imgidx
        datasetSize = len(self.seq)
        print('total images count: ', datasetSize)
        ...
        if self.images_per_identity>0:
            ...
            self.repeat = (datasetSize*5)/(self.images_per_identity*len(self.id2range))
        ...
        if self.triplet_params is not None:
            ...
            if dis_type == 'e':
                ...
                assert self.triplet_alpha<=math.pi * 0.5
        ...
        self.fairface_mode = fairface_mode
        if self.fairface_mode:
            self.fairface_reset()

    def fairface_reset(self):
    ...
    def pairwise_dists(self, embeddings):
        ...
        for idx in range(embeddings.shape[0]):
            ...
            #elif self.dis_type == 'c':
            #    body = mx.nd.broadcast_mul(a_embedding, nd_embedding)
            #    #print(a_embedding.shape, nd_embedding.shape, body.shape)
            #    #print(a_embedding, nd_embedding[0], body[0])
            #    body = mx.nd.sum_axis(body, axis=1)
            #    body = mx.nd.arccos(body)
            #    #print(body.shape)
            #    #print(body[0])
            nd_pdists.append(body)
            if len(nd_pdists)==self.ctx_num or idx==embeddings.shape[0]-1:
                for x in nd_pdists:
                    pdists.append(x.asnumpy().astype('float32'))
                nd_pdists = []
        return pdists
        ...
        while len(self.seq)<self.seq_min_size:
            ...
            while True:
                ...
                for i in range(ba, bb):
                    ...
                    #print(img)
                    #img = self.horizion_flip(img)
                    #img = self.color_aug(img,0.3)
                ...
                net_out = net_out[0].asnumpy().astype('float32')
                #print(net_out)
    ...
    def hard_mining_reset(self):
        ...
        while ba<len(self.oseq):
            ...
            self.mx_model.forward(db, is_train=False)
            net_out = self.mx_model.get_outputs()
            embedding = net_out[0].asnumpy().astype('float32')
    ...
    def pick_random_triplets(self, nrof_images_per_class):
        emb_start_idx = 0
        triplets = []
        people_per_batch = len(nrof_images_per_class)
        seqLen = len(self.triplet_seq)
        ...
        for i in range(people_per_batch):
            ...
            for j in range(1,nrof_images):
                ...
                for pair in range(j, nrof_images): # For every possible positive pair.
                    ...
                    n_idxList = list(range(emb_start_idx)) + list(range(emb_start_idx+nrof_images, seqLen))
                    n_idx = random.choice(n_idxList)
                    triplets.append( (a_idx, p_idx, n_idx) )
            emb_start_idx += nrof_images
        np.random.shuffle(triplets)
        return triplets
    ...
    def select_fairfaces(self):
        ...
        triplets = self.pick_random_triplets(nrof_images_per_class) # shape=(T,3)
        print('found triplets', len(triplets))
        ba = 0
        while True:
            bb = ba+self.per_batch_size//3
    ...
    def reset(self):
        ...
        if self.images_per_identity>0:
            if self.fairface_mode:
                self.select_fairfaces()
            elif self.triplet_mode:
                ...
                for _id in self.id2range:
                    v = self.id2range[_id]
                    idlist.append((_id,list(range(*v))))
    ...
    def color_aug(self, img, x):
        ...
        random.shuffle(augs)
        img = img.asnumpy().astype('float32')
        #print(img.shape)
        for aug in augs:
            #print(img.shape)
            img = aug(img, x)
