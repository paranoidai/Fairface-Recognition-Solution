        ...
        while len(self.seq)<self.seq_min_size:
            ...
            while True:
                ...
                for i in range(ba, bb):
                    ...
                    self.left_images.append(_idx)
                    self.supply_images.append(_idx)
            ...
            for triplet in triplets:
                for i in range(3):
                    _pos = triplet[i]
                    _idx = tag[_pos][1]
                    if(_idx in self.left_images):
                        self.left_images.remove(_idx)

            print('left ids: ', len(self.left_images))
            for item in self.supply_images:
                self.left_images.append(item)
            


            while True:
                bb = ba+self.per_batch_size//4
                if bb>len(triplets):
                    break
                if(len(self.left_images) < self.per_batch_size// 4):
                    break
                ...
                for i in range(self.per_batch_size//4):
                    _idx = self.left_images[0]
                    self.left_images.remove(_idx)
                    self.seq.append(_idx)
                ba = bb
