...
from symbol.dropblock import DropBlock
from symbol.splat import SplitAttentionConv
sys.path.append(os.path.join(os.path.dirname(__file__), '..'))
from config import config
from symbol.symbol_utils import gluon_act
...
class ResNet(HybridBlock):
    ...
    def __init__(self, block, layers, cardinality=1, bottleneck_width=64,
                 classes=1000, dilated=False, dilation=1, norm_layer=nn.BatchNorm,
        ...
        with self.name_scope():
            ...
            self.layer1 = self._make_layer(1, block, 64, layers[0], strides=2, avg_down=avg_down,
                                           norm_layer=norm_layer, last_gamma=last_gamma, use_splat=use_splat, avd=avd)
